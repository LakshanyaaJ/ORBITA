...
class FrameData:
    """Wrapper holding a frame, its metadata, and capture timestamp."""
    __slots__ = ("frame", "frame_id", "timestamp", "width", "height")
...
class LatestFrameBuffer:
    ...
    def push(self, frame: np.ndarray, timestamp: Optional[float] = None) -> int:
        """
        ...
        with self._lock:
            ...
            self._current_frame = FrameData(frame, frame_id, t)
    ...
    def latency_ms(self) -> float:
        """Latency between camera capture of the latest frame and current time."""
        with self._lock:
            if self._current_frame is None:
                return 0.0
            return max(0.0, (time.time() - self._current_frame.timestamp) * 1000.0)
    ...
    def stats(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "latency_ms": round(max(0.0, (time.time() - self._last_push_time) * 1000.0), 1) if self._last_push_time else 0.0,
